    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    /// </summary>
    public OffsetLoop() : this(0, new(new()))
    {
    }
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    ///     <paramref name="ccwPlines" /> or <paramref name="cwPlines" /> or
    ///     <paramref name="plinesIndex" /> is null.
    /// </exception>
    public Shape(List<IndexedPolyline<T>> ccwPlines, List<IndexedPolyline<T>> cwPlines,
        StaticAABB2DIndex<T> plinesIndex)
    {
        ...
        _ccwPlines = new(ccwPlines);
        _cwPlines = new(cwPlines);
    ...
    public static Shape<T> FromPlines(IEnumerable<Polyline<T>> plines)
    {
        ...
        void AddAllBounds(List<IndexedPolyline<T>> list)
        {
            foreach (var pline in list)
            {
                ...
                if (bounds == null) throw new InvalidOperationException("expect non-empty polyline");
        ...
        return new(ccwPlines, cwPlines, plinesIndex);
    }
    ...
    ///     <see
    ///         cref="CreateValidSlicesFromIntersects(List{OffsetLoop{T}}, List{OffsetLoop{T}}, List{SlicePointSet{T}}, T, ShapeOffsetOptions{T})" />
    ///     and
    ...
    public Shape<T> ParallelOffset(T offset, ShapeOffsetOptions<T> options)
    {
        ...
        if (ccwOffsetLoops.Count == 0 && cwOffsetLoops.Count == 0) return Empty();
    ...
    /// </remarks>
    /// <exception cref="ArgumentNullException"><paramref name="options" /> is null.</exception>
    public (List<OffsetLoop<T>> CcwOffsetLoops, List<OffsetLoop<T>> CwOffsetLoops, StaticAABB2DIndex<T> OffsetLoopsIndex
            ...
            ShapeOffsetOptions<T> options)
    {
        ...
        foreach (var pline in CcwPlines)
        {
            ...
            {
                var area = offsetPline.Area();
                if (offset > T.Zero && area < T.Zero) continue;

                var offsetLoop = new OffsetLoop<T>(parentIdx, new(offsetPline));
    ...
    /// </remarks>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="ccwOffsetLoops" /> or <paramref name="cwOffsetLoops" /> or
    ///     <paramref name="offsetLoopsIndex" /> is null.
    /// </exception>
        ...
        T posEqualEps)
    {
        ...
        var offsetLoopCount = ccwOffsetLoops.Count + cwOffsetLoops.Count;
        ...
        for (var i = 0; i < offsetLoopCount; i++)
        {
            ...
            for (var r = 0; r < queryResults.Count; r++)
            {
                var j = queryResults[r];
                if (i == j) continue;

                var reverseKey = ((ulong)(uint)j << 32) | (uint)i;
                if (visitedLoopPairs.Contains(reverseKey)) continue;

                var key = ((ulong)(uint)i << 32) | (uint)j;
                ...
                if (intersects.BasicIntersects.Count == 0 && intersects.OverlappingIntersects.Count == 0) continue;

                var slicePoints = new List<PlineBasicIntersect<T>>();

                foreach (var intr in intersects.BasicIntersects) slicePoints.Add(intr);

                foreach (var overlapIntr in intersects.OverlappingIntersects)
                {
                    var startIndex1 = overlapIntr.StartIndex1;
                    var startIndex2 = overlapIntr.StartIndex2;
                    slicePoints.Add(new(startIndex1, startIndex2, overlapIntr.Point1));
                    slicePoints.Add(new(startIndex1, startIndex2, overlapIntr.Point2));
                }

                slicePointSets.Add(new(i, j, slicePoints));
            }
        }
    ...
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="ccwOffsetLoops" /> or <paramref name="cwOffsetLoops" /> or
    ///     <paramref name="slicePointSets" /> or <paramref name="options" /> is null.
    /// </exception>
        ...
        ShapeOffsetOptions<T> options)
    {
        ...
        for (var setIdx = 0; setIdx < slicePointSets.Count; setIdx++)
        {
        ...
        bool IsSliceValid(in PlineViewData<T> vData, Polyline<T> offsetLoop, int parentIdx, List<int> qStack)
        {
            ...
            var vertexCount = sliceView.VertexCount;
            ...
            var totalPlines = CcwPlines.Count + CwPlines.Count;
            for (var inputLoopIdx = 0; inputLoopIdx < totalPlines; inputLoopIdx++)
            {
                if (inputLoopIdx == parentIdx) continue;
        ...
        for (var loopIdx = 0; loopIdx < offsetLoopCount; loopIdx++)
        {
            sortedIntrs.Clear();
            ...
            {
                foreach (var setIdx in slicePointSetIdxs)
                {
                    var set = slicePointSets[setIdx];
                    var loopIsFirstIndex = set.LoopIdx1 == loopIdx;

                    foreach (var intrPt in set.SlicePoints)
                    {
                        var segIdx = loopIsFirstIndex ? intrPt.StartIndex1 : intrPt.StartIndex2;
                        sortedIntrs.Add(new(segIdx, intrPt.Point));
                    }
                }

                sortedIntrs.Sort((a, b) =>
                {
                    var cmp = a.SegIdx.CompareTo(b.SegIdx);
                    if (cmp != 0) return cmp;

                    var segStart = currLoop.IndexedPline.Polyline.Get(a.SegIdx).Pos();
                    var dist1 = BaseMath.DistSquared(a.Pos, segStart);
                    var dist2 = BaseMath.DistSquared(b.Pos, segStart);
                ...
                if (sortedIntrs.Count == 1)
                {
                    ...
                    if (IsSliceValid(vData, currLoop.IndexedPline.Polyline, currLoop.ParentLoopIdx, queryStack))
                        slicesData.Add(new(loopIdx, vData));
                }
                else
                {
                    for (var w = 0; w < sortedIntrs.Count - 1; w++)
                    {
                        var pt1 = sortedIntrs[w];
                        var pt2 = sortedIntrs[w + 1];
                        var vData = CreateSlice(pt1, pt2, currLoop.IndexedPline.Polyline);
                        if (vData != null && IsSliceValid(vData.Value, currLoop.IndexedPline.Polyline,
                                currLoop.ParentLoopIdx, queryStack)) slicesData.Add(new(loopIdx, vData.Value));
                    }
                    ...
                    if (lastToStartVData != null && IsSliceValid(lastToStartVData.Value, currLoop.IndexedPline.Polyline,
                            currLoop.ParentLoopIdx, queryStack)) slicesData.Add(new(loopIdx, lastToStartVData.Value));
                }
            }
            else
            {
    ...
    /// </remarks>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="slicesData" /> or <paramref name="ccwOffsetLoops" /> or
    ///     <paramref name="cwOffsetLoops" /> is null.
    /// </exception>
        ...
        T sliceJoinEps)
    {
        ...
        if (slicesData.Count == 0) return Empty();
        ...
        for (var i = 0; i < slicesData.Count; i++)
        {
        ...
        for (var sliceIdx = 0; sliceIdx < slicesData.Count; sliceIdx++)
        {
            if (visitedSlicesIdxs[sliceIdx]) continue;
            visitedSlicesIdxs[sliceIdx] = true;

            var currentIndex = sliceIdx;
            var loopCount = 0;
            var maxLoopCount = slicesData.Count;
            ...
            while (true)
            {
                if (loopCount > maxLoopCount)
                    throw new InvalidOperationException(
                        "loopCount exceeded maxLoopCount while stitching slices together");
                loopCount++;
                ...
                if (queryResults.Count == 0)
                {
                    ...
                    var isCcw = currentPline.Orientation() == PlineOrientation.CounterClockwise;
                    if (isCcw)
                        ccwPlinesResult.Add(new(currentPline));
                    else
                        cwPlinesResult.Add(new(currentPline));
                    break;
                }
                ...
                if (nextIndex == -1) nextIndex = queryResults[0];
    ...
    private struct StitchVisitor : IQueryVisitor
    {
        ...
        public bool Visit(int indexPos)
        {
            if (!VisitedSlicesIdxs[indexPos]) QueryResults.Add(indexPos);
            return true;
        }
    }
}