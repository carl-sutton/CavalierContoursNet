...
public struct DelegateQueryVisitor : IQueryVisitor
{
    ...
    /// <returns>
    ///     Whatever the wrapped delegate returned: <see langword="true" /> to continue,
    ///     <see langword="false" /> to stop.
    /// </returns>
    public bool Visit(int indexPos)
    {
        return _delegate(indexPos);
    }
}
...
public struct DelegateNeighborVisitor<T> : INeighborVisitor<T> where T : struct, IFloatingPointIeee754<T>
{
    ...
    ///     <see langword="false" /> to stop.
    /// </returns>
    public bool Visit(int indexPos, T distSquared)
    {
        return _delegate(indexPos, distSquared);
    }
}
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    public StaticAABB2DIndexBuilder(int count, int nodeSize)
    {
        ...
        for (var i = 0; i < numNodes; i++) _indices[i] = i;
        _pos = 0;
    }
    ...
    public StaticAABB2DIndexBuilder<T> Add(T minX, T minY, T maxX, T maxY)
    {
        ...
        _boxes[_pos] = new(minX, minY, maxX, maxY);
        _pos++;
        return this;
    }
    ...
    public StaticAABB2DIndex<T> Build()
    {
        ...
        if (_built) throw new InvalidOperationException("this builder has already been built");
        _built = true;

        if (_pos != _numItems)
            throw new InvalidOperationException(
                $"Added item count should equal static size given to builder (added: {_pos}, expected: {_numItems})");

        if (_numItems == 0) return new(_nodeSize, _numItems, _levelBounds, Array.Empty<AABB<T>>(), _indices);

        // Calculate total bounds
        var minX = _boxes[0].MinX;
        var minY = _boxes[0].MinY;
        var maxX = _boxes[0].MaxX;
        var maxY = _boxes[0].MaxY;
        ...
        if (_numItems <= _nodeSize)
        {
            ...
            return new(_nodeSize, _numItems, _levelBounds, _boxes, _indices);
        }

        var width = double.CreateChecked(maxX - minX);
        var height = double.CreateChecked(maxY - minY);
        var extentMinX = double.CreateChecked(minX);
        var extentMinY = double.CreateChecked(minY);

        double hilbertMax = ushort.MaxValue;
        var scaledWidth = hilbertMax / width;
        var scaledHeight = hilbertMax / height;

        var hilbertValues = new uint[_numItems];
        for (var i = 0; i < _numItems; i++)
        {
            var aabbMinX = double.CreateChecked(_boxes[i].MinX);
            var aabbMinY = double.CreateChecked(_boxes[i].MinY);
            var aabbMaxX = double.CreateChecked(_boxes[i].MaxX);
            var aabbMaxY = double.CreateChecked(_boxes[i].MaxY);

            var x = HilbertCoord(scaledWidth, aabbMinX, aabbMaxX, extentMinX);
            var y = HilbertCoord(scaledHeight, aabbMinY, aabbMaxY, extentMinY);
        ...
        if (!allSameHilbertValue) RadixSort(hilbertValues, _boxes, _indices, 0, _numItems - 1, _nodeSize, 1u << 31);

        var pos = 0;
        for (var levelIdx = 0; levelIdx < _levelBounds.Length - 1; levelIdx++)
        {
            var levelEnd = _levelBounds[levelIdx];
            while (pos < levelEnd)
            {
                var nodeMinX = T.MaxValue;
                var nodeMinY = T.MaxValue;
                var nodeMaxX = T.MinValue;
                var nodeMaxY = T.MinValue;
                var nodeIndex = pos;
                ...
                _boxes[_pos] = new(nodeMinX, nodeMinY, nodeMaxX, nodeMaxY);
                _pos++;
            }
        }
    ...
    {
        var value = scaledExtent * (0.5 * (aabbMin + aabbMax) - extentMin);
    ...
    public static uint HilbertXyToIndex(ushort x, ushort y)
    {
        ...
        var a2 = a1 | (b1 >> 1);
        var b2 = (a1 >> 1) ^ a1;
        var c2 = (c1 >> 1) ^ (b1 & (d1 >> 1)) ^ c1;
        var d2 = (a1 & (c1 >> 1)) ^ (d1 >> 1) ^ d1;
        ...
        uint bit)
    {
        ...
        while (true)
        {
            ...
            if (left / nodeSize >= right / nodeSize || bit == 0) return;
            ...
            while (i < j)
            {
                while (i < j && (values[i] & bit) == 0) i++;
                while (i < j && (values[j - 1] & bit) != 0) j--;
                if (i == j) break;
            ...
            if (i == left || i == end)
            {
                ...
                if (emptyPartitions == 2)
                {
                    ...
                    for (var k = left + 1; k <= right; k++) differingBits |= first ^ values[k];
                    if (differingBits == 0) return;
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    /// </remarks>
    public ReadOnlySpan<AABB<T>> ItemBoxes => _boxes.AsSpan(0, Count);

    /// <summary>
    ///     Maps a position in <see cref="ItemBoxes" /> back to the index position the item was added
    ///     at.
    /// </summary>
    public ReadOnlySpan<int> ItemIndices => _indices.AsSpan(0, Count);
    ...
    public List<int> Query(T minX, T minY, T maxX, T maxY)
    {
        ...
        var visitor = new DelegateQueryVisitor(i =>
        {
            results.Add(i);
            return true;
        });
    ...
    public IEnumerable<int> QueryIter(T minX, T minY, T maxX, T maxY)
    {
        ...
        var nodeIndex = _boxes.Length - 1;
        var pos = nodeIndex;
        var level = _levelBounds.Length - 1;
        var end = Math.Min(nodeIndex + NodeSize, _levelBounds[level]);
        ...
        while (true)
        {
            while (pos < end)
            {
                ...
                var aabb = _boxes[currentPos];
                if (!aabb.Overlaps(minX, minY, maxX, maxY)) continue;

                var index = _indices[currentPos];
                if (nodeIndex < Count)
                {
                    yield return index;
                }
            ...
            if (stack.Count > 1)
            {
                ...
                stack.RemoveAt(stack.Count - 1);
                pos = nodeIndex;
                end = Math.Min(nodeIndex + NodeSize, _levelBounds[level]);
            }
            else
            {
                break;
    ...
    public bool VisitQueryWithStack<V>(T minX, T minY, T maxX, T maxY, ref V visitor, List<int> stack)
        where V : struct, IQueryVisitor
    {
    ...
    private bool VisitQueryWithStackImpl<V>(T minX, T minY, T maxX, T maxY, ref V visitor, List<int> stack)
        where V : struct, IQueryVisitor
    {
        var nodeIndex = _boxes.Length - 1;
        var level = _levelBounds.Length - 1;
    ...
    public bool VisitNeighborsWithQueue<V>(T x, T y, ref V visitor, PriorityQueue<NeighborsState, T> queue)
        where V : struct, INeighborVisitor<T>
    {
    ...
    private bool VisitNeighborsWithQueueImpl<V>(T x, T y, ref V visitor, PriorityQueue<NeighborsState, T> queue)
        where V : struct, INeighborVisitor<T>
    {
        ...
        while (true)
        {
            var upperIdx = Array.BinarySearch(_levelBounds, nodeIndex);
            if (upperIdx >= 0) upperIdx += 1;
            else upperIdx = ~upperIdx;

            var end = Math.Min(nodeIndex + NodeSize, _levelBounds[upperIdx]);

            for (var pos = nodeIndex; pos < end; pos++)
            {
                var aabb = _boxes[pos];
                var dx = AxisDist(x, aabb.MinX, aabb.MaxX);
                var dy = AxisDist(y, aabb.MinY, aabb.MaxY);
                var dist = dx * dx + dy * dy;
                var index = _indices[pos];
                var isLeafNode = nodeIndex < Count;
                queue.Enqueue(new(index, isLeafNode), dist);
            }

            var continueSearch = false;
            while (queue.TryDequeue(out var state, out var dist))
                if (state.IsLeafNode)
                {
