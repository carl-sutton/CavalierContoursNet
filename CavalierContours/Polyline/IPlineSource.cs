...
public static class PlineSourceExtensions
{
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        for (var i = 0; i < vc - 1; i++) yield return (pline.Get(i), pline.Get(i + 1));

        if (pline.IsClosed) yield return (pline.Get(vc - 1), pline.Get(0));
    }
    ...
    {
        var vc = pline.VertexCount;
        for (var i = 0; i < vc; i++) yield return pline.Get(i);
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        for (var i = 0; i < vc - 1; i++) yield return (i, i + 1);

        if (pline.IsClosed) yield return (vc - 1, 0);
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        for (var i = 0; i < vc; i++)
            if (!self.Get(i).FuzzyEqEps(other.Get(i), eps))
                return false;
        return true;
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        foreach (var (sv1, sv2) in pline.IterSegments())
            ...
            {
                var arcBox = PlineSeg.ArcSegBoundingBox(sv1, sv2);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var len = T.Zero;
        foreach (var (v1, v2) in pline.IterSegments()) len += PlineSeg.SegLength(v1, v2);
        return len;
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        foreach (var (v1, v2) in pline.IterSegments())
        {
            ...
            if (!v1.BulgeIsZero())
            {
                var b = T.Abs(v1.Bulge);
                var sweepAngle = BaseMath.AngleFromBulge(b);
                var triangleBase = (v2.Pos() - v1.Pos()).Length();
                var radius = triangleBase * ((b * b + T.One) / (four * b));
                var sagitta = b * triangleBase / two;
                var triangleHeight = radius - sagitta;
                var doubleSectorArea = sweepAngle * radius * radius;
                var doubleTriangleArea = triangleBase * triangleHeight;
                var doubleArcArea = doubleSectorArea - doubleTriangleArea;
                if (v1.BulgeIsNeg()) doubleArcArea = -doubleArcArea;
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var prevPos = pline.Get(0).Pos();

        for (var i = 1; i < vc; i++)
        {
            var v = pline.Get(i);
            var isRepeat = v.Pos().FuzzyEqEps(prevPos, posEqualEps);

            if (isRepeat)
            {
                if (result == null)
                {
                    result = new(pline.IsClosed);
                    for (var j = 0; j < i; j++) result.AddVertex(pline.Get(j));
                }
        ...
        if (pline.IsClosed && pline.Last()!.Value.Pos().FuzzyEqEps(pline.Get(0).Pos(), posEqualEps))
        {
            ...
            {
                result = new(pline.IsClosed);
                for (var j = 0; j < vc; j++) result.AddVertex(pline.Get(j));
            }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var segCount = pline.IsClosed ? vc : vc - 1;
        var builder = new StaticAABB2DIndexBuilder<T>(segCount);

        foreach (var (v1, v2) in pline.IterSegments())
        {
            var approxAabb = PlineSeg.SegFastApproxBoundingBox(v1, v2);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        foreach (var (v1, v2) in pline.IterSegments())
        {
            var actualAabb = PlineSeg.SegBoundingBox(v1, v2);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var firstPos = pline.Get(0).Pos();
        if (pline.VertexCount == 1)
        {
            var dist = (firstPos - point).Length();
            return new ClosestPointResult<T>(0, firstPos, dist);
        }

        var bestIndex = 0;
        var bestPoint = firstPos;
        var bestDistSq = T.MaxValue;

        foreach (var (i, j) in pline.IterSegmentIndexes())
        {
            var v1 = pline.Get(i);
            var v2 = pline.Get(j);
            var cp = PlineSeg.SegClosestPoint(v1, v2, point, posEqualEps);
            var distSq = (point - cp).LengthSquared();
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description><c>1</c> — the polyline winds around the point counter-clockwise.</description>
    ///         </item>
    ///         <item>
    ///             <description><c>0</c> — the point lies outside the polyline.</description>
    ///         </item>
    ///         <item>
    ///             <description><c>-1</c> — the polyline winds around the point clockwise.</description>
    ///         </item>
    ///     </list>
    ///     <para>
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        int ProcessLineWinding(PlineVertex<T> v1, PlineVertex<T> v2, Vector2<T> pt)
        {
            ...
            if (v1.Y <= pt.Y)
            {
                if (v2.Y > pt.Y && BaseMath.IsLeft(v1.Pos(), v2.Pos(), pt)) r += 1;
            }
        ...
        {
            var isCcw = v1.BulgeIsPos();
            var pointIsLeft = isCcw
                ? BaseMath.IsLeft(v1.Pos(), v2.Pos(), pt)
                : BaseMath.IsLeftOrEqual(v1.Pos(), v2.Pos(), pt);

            bool DistToCenterLessThanRadius()
            {
                var (arcRadius, arcCenter) = PlineSeg.SegArcRadiusAndCenter(v1, v2);
                var dist2 = BaseMath.DistSquared(arcCenter, pt);
            ...
            if (v1.Y <= pt.Y)
            {
                ...
                else
                {
                    ...
                    else if (!isCcw && pointIsLeft && v1.X < pt.X && pt.X < v2.X &&
                             DistToCenterLessThanRadius()) r -= 1;
                }
            }
            else if (v2.Y <= pt.Y)
            {
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var absError = T.Abs(errorDistance);
        ...
        foreach (var (v1, v2) in pline.IterSegments())
        {
            ...
            var startAngle = BaseMath.Angle(arcCenter, v1.Pos());
            var endAngle = BaseMath.Angle(arcCenter, v2.Pos());
            var angleDiff = T.Abs(BaseMath.DeltaAngle(startAngle, endAngle));

            var two = T.CreateChecked(2);
            // abs goes outside acos, matching upstream (one - abs_error/arc_radius).acos().abs()
            var segSubAngle = two * T.Abs(T.Acos(T.One - absError / arcRadius));
            var segCount = T.Ceiling(angleDiff / segSubAngle);
            var segAngleOffset = v1.BulgeIsNeg() ? -angleDiff / segCount : angleDiff / segCount;

            result.Add(v1.X, v1.Y, T.Zero);
            var intCount = int.CreateChecked(segCount);
            for (var i = 1; i < intCount; i++)
            {
                var anglePos = T.CreateChecked(i);
                var angle = anglePos * segAngleOffset + startAngle;
                var pos = BaseMath.PointOnCircle(arcRadius, arcCenter, angle);
                result.Add(pos.X, pos.Y, T.Zero);
            }
        }

        if (!pline.IsClosed) result.AddVertex(pline.Last()!.Value);
    ...
    /// </exception>
    /// <exception cref="ArgumentNullException"><paramref name="pline" /> is null.</exception>
    public static (bool Success, int SegIndex, Vector2<T> Point, T AccLength) FindPointAtPathLength<T>(
        this IPlineSource<T> pline, T targetPathLength)
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ArgumentNullException.ThrowIfNull(pline);

        if (targetPathLength <= T.Zero) return (true, 0, pline.Get(0).Pos(), T.Zero);

        var accLength = T.Zero;
        var i = 0;
        foreach (var (v1, v2) in pline.IterSegments())
        {
            var segLen = PlineSeg.SegLength(v1, v2);
            var sumLen = accLength + segLen;
            ...
            var t = (targetPathLength - accLength) / segLen;

            if (v1.BulgeIsZero())
            {
                var pt = v1.Pos() + (v2.Pos() - v1.Pos()).Scale(t);
                return (true, i, pt, accLength);
            }
            else
            {
                var (radius, center) = PlineSeg.SegArcRadiusAndCenter(v1, v2);
                var startAngle = BaseMath.Angle(center, v1.Pos());
                var totalSweepAngle = BaseMath.AngleFromBulge(v1.Bulge);
                var targetAngle = startAngle + totalSweepAngle * t;

                var pt = BaseMath.PointOnCircle(radius, center, targetAngle);
                return (true, i, pt, accLength);
            }
        }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var otherCount = other.VertexCount;
        for (var i = 0; i < otherCount; i++) self.AddOrReplaceVertex(other.Get(i), posEqualEps);
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var plineCount = pline.VertexCount;
        for (var i = 0; i < plineCount; i++) result.AddOrReplaceVertex(pline.Get(i), posEqualEps);

        if (pline.IsClosed && result.VertexCount >= 2)
        {
            var last = result.Get(result.VertexCount - 1);
            if (last.Pos().FuzzyEqEps(result.Get(0).Pos(), posEqualEps)) result.Remove(result.VertexCount - 1);
        }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var result = new O();
        result.SetIsClosed(pline.IsClosed);
        var count = pline.VertexCount;
        for (var i = 0; i < count; i++) result.AddVertex(pline.Get(i));
        result.SetUserDataValues(pline.UserDataValues);
        return result;
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        {
            if (v2_val.Pos().FuzzyEqEps(v3_val.Pos(), posEqualEps)) return true;

            var collinear = (v1_val.X * (v2_val.Y - v3_val.Y) + v2_val.X * (v3_val.Y - v1_val.Y) +
                             v3_val.X * (v1_val.Y - v2_val.Y))
                .FuzzyEqZero(posEqualEps);
            var sameDirection = (v3_val.Pos() - v2_val.Pos()).Dot(v2_val.Pos() - v1_val.Pos()) > -posEqualEps;
        ...
        Polyline<T> CopySelf(int count)
        {
            ...
            for (var idx = 0; idx < count; idx++) pl.AddVertex(self.Get(idx));
            return pl;
        }
        ...
        var v1BulgeIsZero = v1.BulgeIsZero();
        var v2BulgeIsZero = v2.BulgeIsZero();
        var v1BulgeIsPos = v1.BulgeIsPos();
        var v2BulgeIsPos = v2.BulgeIsPos();

        var iterCount = self.IsClosed ? vc - 1 : vc - 2;
        var enumIndex = i;

        for (var step = 0; step < iterCount; step++, enumIndex++)
        {
            var v3 = self.Get(enumIndex % vc);
            RedundantCase state;
            var computedBulge = T.Zero;
            ...
            {
                var isFinalVertexForOpen = !self.IsClosed && enumIndex == vc;
            ...
            {
                if (!v1_v2_arc.HasValue) v1_v2_arc = PlineSeg.SegArcRadiusAndCenter(v1, v2);
                ...
                {
                    var angle1 = BaseMath.Angle(arcCenter1, v1.Pos());
                    var angle2 = BaseMath.Angle(arcCenter1, v2.Pos());
                    var angle3 = BaseMath.Angle(arcCenter1, v3.Pos());
                    var totalSweep = T.Abs(BaseMath.DeltaAngle(angle1, angle2)) +
                                     T.Abs(BaseMath.DeltaAngle(angle2, angle3));

                    var two = T.CreateChecked(2);
                    var avgRadius = (arcRadius1 + arcRadius2) / two;

                    if ((avgRadius * totalSweep).FuzzyLt(avgRadius * T.Pi, posEqualEps))
                    {
                        computedBulge = v1BulgeIsPos
                            ? BaseMath.BulgeFromAngle(totalSweep)
                            : -BaseMath.BulgeFromAngle(totalSweep);
            ...
            switch (state)
            {
                case RedundantCase.IncludeVertex:
                    if (result != null) result.AddVertex(v2);
                    v1 = v2;
                    v2 = v3;
                    v1_v2_arc = null;
                ...
                case RedundantCase.DiscardVertex:
                    if (result == null) result = CopySelf(enumIndex - 1);
                    v2 = v3;
                    v1_v2_arc = null;
                    v2BulgeIsZero = v3.BulgeIsZero();
                    v2BulgeIsPos = v3.BulgeIsPos();
        ...
        if (self.IsClosed)
        {
            ...
            {
                if (result == null) result = CopySelf(vc);
    ...
    /// </returns>
    /// <exception cref="ArgumentNullException"><paramref name="self" /> is null.</exception>
    public static Polyline<T>? RotateStart<T>(this IPlineSource<T> self, int startIndex, Vector2<T> point,
        T posEqualEps)
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ArgumentNullException.ThrowIfNull(self);
        ...
        var vc = self.VertexCount;
        if (!self.IsClosed || vc < 2 || startIndex < 0 || startIndex > vc - 1) return null;

        IEnumerable<PlineVertex<T>> WrappingVertexesStartingAt(int start)
        {
            for (var idx = start; idx < vc; idx++) yield return self.Get(idx);
            for (var idx = 0; idx < start; idx++) yield return self.Get(idx);
        }
        ...
        {
            var nextIndex = self.NextWrappingIndex(startIndex);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var firstBulge = self.Get(0).Bulge;
        for (var i = 1; i < vc; i++)
        {
            var b = -self.Get(i).Bulge;
            self.SetVertex(i - 1, self.Get(i - 1).WithBulge(b));
        }

        if (self.IsClosed) self.SetVertex(vc - 1, self.Get(vc - 1).WithBulge(-firstBulge));
    }
