...
public static class PlineOffset
{
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        {
            var (arcRadius, arcCenter) = PlineSeg.SegArcRadiusAndCenter(v1, v2);
            var offs = v1.BulgeIsNeg() ? offset : -offset;
            var radiusAfterOffset = arcRadius + offs;
    ...
    private static T BulgeForConnection<T>(
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        var a1 = BaseMath.Angle(arcCenter, startPoint);
        var a2 = BaseMath.Angle(arcCenter, endPoint);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var sp = s1.V2.Pos();
        var ep = s2.V1.Pos();
        var bulge = BulgeForConnection(arcCenter, sp, ep, connectionArcsCcw);
    ...
    private static void AddOrReplace<T>(IPlineSourceMut<T> self, T x, T y, T bulge, T posEqualEps)
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        self.AddOrReplaceVertex(new(x, y, bulge), posEqualEps);
    }
    ...
    private static void LineLineJoin<T>(
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        var connectionArcsCcw = joinParams.ConnectionArcsCcw;
        var posEqualEps = joinParams.PosEqualEps;
        var v1 = s1.V1;
        var v2 = s1.V2;
        var u1 = s2.V1;
        var u2 = s2.V2;
        ...
        else
        {
            ...
            switch (intr.Kind)
            {
                case LineLineIntrKind.NoIntersect:
                {
                    var sp = s1.V2.Pos();
                    var ep = s2.V1.Pos();
                    var bulge = connectionArcsCcw ? T.One : -T.One;
                    AddOrReplace(result, sp.X, sp.Y, bulge, posEqualEps);
                    AddOrReplace(result, ep.X, ep.Y, s2.V1.Bulge, posEqualEps);
                    break;
                }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        Debug.Assert(v1.BulgeIsZero() && !u1.BulgeIsZero(),
            "first segment should be line, second segment should be arc");

        var (arcRadius, arcCenter) = PlineSeg.SegArcRadiusAndCenter(u1, u2);

        void ProcessIntersect(T t, Vector2<T> intersect)
        {
            var trueLineIntr = !IsFalseIntersect(t);
            var trueArcIntr = BaseMath.PointWithinArcSweep(
                arcCenter,
                u1.Pos(),
                u2.Pos(),
                u1.BulgeIsNeg(),
            ...
            {
                var a = BaseMath.Angle(arcCenter, intersect);
                var arcEndAngle = BaseMath.Angle(arcCenter, u2.Pos());
                var theta = BaseMath.DeltaAngle(a, arcEndAngle);
        ...
        switch (lineCircleIntr.Kind)
        {
            ...
            {
                ProcessIntersect(lineCircleIntr.T0,
                    BaseMath.PointFromParametric(v1.Pos(), v2.Pos(), lineCircleIntr.T0));
                break;
            }
            case LineCircleIntrKind.TwoIntersects:
            {
                var intr1 = BaseMath.PointFromParametric(v1.Pos(), v2.Pos(), lineCircleIntr.T0);
                var dist1 = BaseMath.DistSquared(intr1, s1.OrigV2Pos);
                var intr2 = BaseMath.PointFromParametric(v1.Pos(), v2.Pos(), lineCircleIntr.T1);
                var dist2 = BaseMath.DistSquared(intr2, s1.OrigV2Pos);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        void ProcessIntersect(T t, Vector2<T> intersect)
        {
            ...
            if (trueLineIntr && trueArcIntr)
            {
                ...
                {
                    var a = BaseMath.Angle(arcCenter, intersect);
                    var (_, prevArcCenter) = PlineSeg.SegArcRadiusAndCenter(prevVertex, v2);
                    var prevArcStartAngle = BaseMath.Angle(prevArcCenter, prevVertex.Pos());
                    var updatedPrevTheta = BaseMath.DeltaAngle(prevArcStartAngle, a);

                    if (updatedPrevTheta > T.Zero == prevVertex.BulgeIsPos())
                        result.SetVertex(result.VertexCount - 1,
                            prevVertex.WithBulge(BaseMath.BulgeFromAngle(updatedPrevTheta)));
                }
        ...
        switch (lineCircleIntr.Kind)
        {
            ...
            {
                ProcessIntersect(lineCircleIntr.T0,
                    BaseMath.PointFromParametric(u1.Pos(), u2.Pos(), lineCircleIntr.T0));
                break;
            }
            case LineCircleIntrKind.TwoIntersects:
            {
                ...
                var dist1 = BaseMath.DistSquared(intr1, origPoint);
                var intr2 = BaseMath.PointFromParametric(u1.Pos(), u2.Pos(), lineCircleIntr.T1);
                var dist2 = BaseMath.DistSquared(intr2, origPoint);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var (arc1Radius, arc1Center) = PlineSeg.SegArcRadiusAndCenter(v1, v2);
        var (arc2Radius, arc2Center) = PlineSeg.SegArcRadiusAndCenter(u1, u2);
        ...
        void ProcessIntersect(Vector2<T> intersect, bool trueIntersect)
        {
            ...
            else
            {
                ...
                if (!prevVertex.BulgeIsZero()
                    && !prevVertex.Pos().FuzzyEqEps(v2.Pos(), posEqualEps))
                {
                    var a1 = BaseMath.Angle(arc1Center, intersect);
                    var (_, prevArcCenter) = PlineSeg.SegArcRadiusAndCenter(prevVertex, v2);
                    var prevArcStartAngle = BaseMath.Angle(prevArcCenter, prevVertex.Pos());
                    var updatedPrevTheta = BaseMath.DeltaAngle(prevArcStartAngle, a1);
                ...
                var a2 = BaseMath.Angle(arc2Center, intersect);
                var endAngle = BaseMath.Angle(arc2Center, u2.Pos());
                var theta = BaseMath.DeltaAngle(a2, endAngle);
        ...
        switch (ccIntr.Kind)
        {
            ...
            {
                var dist1 = BaseMath.DistSquared(ccIntr.Point1, s1.OrigV2Pos);
                var dist2 = BaseMath.DistSquared(ccIntr.Point2, s1.OrigV2Pos);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var vc = polyline.VertexCount;
        if (vc < 2) return new();

        var rawOffsetSegs = CreateUntrimmedRawOffsetSegs(polyline, offset);
        if (rawOffsetSegs.Count == 0) return new();

        // detect single collapsed arc segment
        if (rawOffsetSegs.Count == 1 && rawOffsetSegs[0].CollapsedArc) return new();

        var connectionArcsCcw = offset < T.Zero;
        ...
        {
            var s1IsLine = s1.V1.BulgeIsZero();
            var s2IsLine = s2.V1.BulgeIsZero();
        ...
        if (rawOffsetSegs.Count >= 2) JoinSegPair(rawOffsetSegs[0], rawOffsetSegs[1], result);

        var firstVertexReplaced = result.VertexCount == 1;

        for (var i = 1; i < rawOffsetSegs.Count - 1; i++) JoinSegPair(rawOffsetSegs[i], rawOffsetSegs[i + 1], result);
        ...
        if (polyline.IsClosed && result.VertexCount > 1)
        {
            ...
            for (var idx = 1; idx < closingPartResult.VertexCount; idx++) result.AddVertex(closingPartResult.Get(idx));
            ...
            if (!firstVertexReplaced)
            {
                ...
                if (result.Get(0).BulgeIsZero())
                {
                    ...
                    var b = result.Get(0).Bulge;
                    result.SetVertex(0, new(updatedFirstPos.X, updatedFirstPos.Y, b));
                }
                else if (result.VertexCount > 1)
                {
                    // update position and bulge
                    var (_, arcCenter) = PlineSeg.SegArcRadiusAndCenter(result.Get(0), result.Get(1));
                    var a1 = BaseMath.Angle(arcCenter, updatedFirstPos);
                    var a2 = BaseMath.Angle(arcCenter, result.Get(1).Pos());
                    var updatedTheta = BaseMath.DeltaAngle(a1, a2);
                    ...
                    else
                    {
                        // update position and bulge
                        result.SetVertex(0,
                            new(updatedFirstPos.X, updatedFirstPos.Y, BaseMath.BulgeFromAngle(updatedTheta)));
                    }
                }
            }
        ...
        if (result.VertexCount == 1) result.Clear();
    ...
    ///     <paramref name="polyline" /> or <paramref name="aabbIndex" /> or
    ///     <paramref name="queryStack" /> is null.
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var absOffset = T.Abs(offset) - offsetTol;
        var minDist = absOffset * absOffset;
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (slice.EndIndexOffset == 0)
        {
            ...
            if (!PointValidDist(v1.Pos())) return false;
            var v2 = PlineVertex<T>.FromVector2(slice.EndPoint, T.Zero);
            if (!PointValidDist(v2.Pos())) return false;
            var midpoint = PlineSeg.SegMidpoint(v1, v2);
            if (!PointValidDist(midpoint)) return false;
        ...
        if (!PointValidDist(startSegMidpoint)) return false;

        var endIndex = rawOffsetPolyline.FwdWrappingIndex(slice.StartIndex, slice.EndIndexOffset);
        ...
        if (!PointValidDist(endSegMidpoint)) return false;
    ...
    ///     <see
    ///         cref="SlicesFromDualRawOffsets{T}(IPlineSource{T}, IPlineSource{T}, IPlineSource{T}, StaticAABB2DIndex{T}, T, PlineOffsetOptions{T})" />
    ///     otherwise.
    /// </remarks>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="originalPolyline" /> or <paramref name="rawOffsetPolyline" />
    ///     or <paramref name="origPolylineIndex" /> or <paramref name="options" /> is null.
    /// </exception>
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (rawOffsetPolyline.VertexCount < 2) return result;

        var posEqualEps = options.PosEqualEps;
        var offsetDistEps = options.OffsetDistEps;
        ...
        foreach (var kvp in intersectsLookup)
        {
            ...
            {
                var dist1 = BaseMath.DistSquared(si1, startPos);
                var dist2 = BaseMath.DistSquared(si2, startPos);
        ...
        foreach (var kvp in intersectsLookup)
        {
            var startIndex = kvp.Key;
            var intrList = kvp.Value;

            for (var idx = 0; idx < intrList.Count - 1; idx++)
            {
                ...
                if (slice.HasValue && SliceIsValid(slice.Value, originalPolyline, rawOffsetPolyline, origPolylineIndex,
                        offset, queryStack, posEqualEps, offsetDistEps)) result.Add(slice.Value);
            }

            var nextIndex = rawOffsetPolyline.NextWrappingIndex(startIndex);
            ...
            if (wrapSlice.HasValue && SliceIsValid(wrapSlice.Value, originalPolyline, rawOffsetPolyline,
                    origPolylineIndex, offset, queryStack, posEqualEps, offsetDistEps)) result.Add(wrapSlice.Value);
        }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        foreach (var startIndex in queryResults)
        {
    ...
    ///     <see
    ///         cref="SlicesFromRawOffset{T}(IPlineSource{T}, IPlineSource{T}, StaticAABB2DIndex{T}, T, PlineOffsetOptions{T})" />
    ///     .
    /// </remarks>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="originalPolyline" /> or <paramref name="rawOffsetPolyline" />
    ///     or <paramref name="dualRawOffsetPolyline" /> or <paramref name="origPolylineIndex" /> or
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        foreach (var intr in dualIntrs.BasicIntersects) AddIntr(intr.StartIndex1, intr.Point);
        ...
        if (!originalPolyline.IsClosed)
        {
            ...
            firstEntry.MoveNext();
            var intrIdx = firstEntry.Current.Key;
        ...
        foreach (var kvp in intersectsLookup)
        {
            ...
            if (nextIntrList == null)
            {
                ...
                else
                {
                    ...
                    if (slice.HasValue && SliceIsValid(slice.Value, originalPolyline, rawOffsetPolyline,
                            origPolylineIndex, offset, queryStack, posEqualEps, offsetDistEps)) result.Add(slice.Value);
                    return result;
                }
            }
    ...
    /// </remarks>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="rawOffsetPline" /> or <paramref name="slices" /> or
    ///     <paramref name="options" /> is null.
    /// </exception>
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var joinEps = options.SliceJoinEps;
        var posEqualEps = options.PosEqualEps;
        ...
        var aabbIndex = new Func<StaticAABB2DIndex<T>>(() =>
        {
            ...
            for (var idx = 0; idx < slices.Count; idx++)
            {
        ...
        for (var i = 0; i < slices.Count; i++)
        {
            ...
            while (true)
            {
                ...
                var currentLoopStartIndex = currentSlice.StartIndex;
                ...
                {
                    var slice = slices[idx];
                    if (currentLoopStartIndex <= slice.StartIndex) return slice.StartIndex - currentLoopStartIndex;
                ...
                {
                    var cmp = GetIndexDist(a).CompareTo(GetIndexDist(b));
                    if (cmp != 0) return cmp;
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (rawOffset.IsEmpty()) return new();

        if (polyline.IsClosed && !options.HandleSelfIntersects)
        {
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (polyline.VertexCount < 2) return new();
        ...
        foreach (var cursor in result) cursor.SetUserDataValues(polyline.UserDataValues);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        public bool Visit(int i)
        {
            var j = _originalPolyline.NextWrappingIndex(i);
    ...
    private struct StitchVisitor : IQueryVisitor
    {
        ...
        public bool Visit(int i)
        {
            if (!_visitedIndexes[i]) _queryResults.Add(i);
            return true;
        }
    }
}