...
public static class PlineContains
{
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        bool PointInPline1(Vector2<T> point)
        {
            return pline1.WindingNumber(point) != 0;
        }

        bool PointInPline2(Vector2<T> point)
        {
            return pline2.WindingNumber(point) != 0;
        }

        bool IsPline1InPline2()
        {
            return PointInPline2(pline1.Get(0).Pos());
        }

        bool IsPline2InPline1()
        {
            return PointInPline1(pline2.Get(0).Pos());
        }
        ...
        if (PlineIntersects.ScanForIntersect(pline1, pline2, findOptions)) return PlineContainsResult.Intersected;

        if (IsPline2InPline1()) return PlineContainsResult.Pline2InsidePline1;

        if (IsPline1InPline2()) return PlineContainsResult.Pline1InsidePline2;
