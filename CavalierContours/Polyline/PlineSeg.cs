...
public static class PlineSeg
{
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        var absBulge = T.Abs(v1.Bulge);
        var chordV = v2.Pos() - v1.Pos();
        var chordLen = chordV.Length();
        var four = T.CreateChecked(4);
        var two = T.CreateChecked(2);
        var radius = chordLen * (absBulge * absBulge + T.One) / (four * absBulge);

        var s = absBulge * chordLen / two;
        var m = radius - s;
        var offsX = -m * chordV.Y / chordLen;
        var offsY = m * chordV.X / chordLen;
        if (v1.BulgeIsNeg())
        {
            offsX = -offsX;
            offsY = -offsY;
    ...
    /// </returns>
    public static SplitResult<T> SegSplitAtPoint<T>(PlineVertex<T> v1, PlineVertex<T> v2, Vector2<T> pointOnSeg,
        T posEqualEps)
        where T : struct, IFloatingPointIeee754<T>
    {
        if (v1.BulgeIsZero())
        {
            ...
            return new(updatedStart, splitVertex);
        }
        ...
        var (_, arcCenter) = SegArcRadiusAndCenter(v1, v2);

        var pointPosAngle = BaseMath.Angle(arcCenter, pointOnSeg);
        var arcStartAngle = BaseMath.Angle(arcCenter, v1.Pos());
        var theta1 = BaseMath.DeltaAngleSigned(arcStartAngle, pointPosAngle, v1.BulgeIsNeg());
        var bulge1 = BaseMath.BulgeFromAngle(theta1);

        var arcEndAngle = BaseMath.Angle(arcCenter, v2.Pos());
        var theta2 = BaseMath.DeltaAngleSigned(pointPosAngle, arcEndAngle, v1.BulgeIsNeg());
        var bulge2 = BaseMath.BulgeFromAngle(theta2);

        return new(
            new(v1.X, v1.Y, bulge1),
            new(pointOnSeg.X, pointOnSeg.Y, bulge2)
        );
    }
    ...
    public static Vector2<T> SegTangentVector<T>(PlineVertex<T> v1, PlineVertex<T> v2, Vector2<T> pointOnSeg)
        where T : struct, IFloatingPointIeee754<T>
    {
        if (v1.BulgeIsZero()) return v2.Pos() - v1.Pos();
    ...
    public static Vector2<T> SegClosestPoint<T>(PlineVertex<T> v1, PlineVertex<T> v2, Vector2<T> point, T epsilon)
        where T : struct, IFloatingPointIeee754<T>
    {
        if (v1.BulgeIsZero()) return BaseMath.LineSegClosestPoint(v1.Pos(), v2.Pos(), point);

        var (arcRadius, arcCenter) = SegArcRadiusAndCenter(v1, v2);
        if (point.FuzzyEqEps(arcCenter, epsilon)) return v1.Pos();

        if (BaseMath.PointWithinArcSweep(arcCenter, v1.Pos(), v2.Pos(), v1.BulgeIsNeg(), point, epsilon))
        {
            var vToPoint = (point - arcCenter).Normalize();
            return vToPoint.Scale(arcRadius) + arcCenter;
        }

        var dist1 = BaseMath.DistSquared(v1.Pos(), point);
        var dist2 = BaseMath.DistSquared(v2.Pos(), point);
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        if (v1.BulgeIsZero())
        {
            var (xMin, xMax) = BaseMath.MinMax(v1.X, v2.X);
            var (yMin, yMax) = BaseMath.MinMax(v1.Y, v2.Y);
            return new(xMin, yMin, xMax, yMax);
        }

        var b = v1.Bulge;
        var offsX = b * (v2.Y - v1.Y) / two;
        var offsY = -b * (v2.X - v1.X) / two;

        var (ptXMin, ptXMax) = BaseMath.MinMax(v1.X + offsX, v2.X + offsX);
        var (ptYMin, ptYMax) = BaseMath.MinMax(v1.Y + offsY, v2.Y + offsY);

        var (endPointXMin, endPointXMax) = BaseMath.MinMax(v1.X, v2.X);
        var (endPointYMin, endPointYMax) = BaseMath.MinMax(v1.Y, v2.Y);

        var minX = T.Min(endPointXMin, ptXMin);
        var minY = T.Min(endPointYMin, ptYMin);
        var maxX = T.Max(endPointXMax, ptXMax);
        var maxY = T.Max(endPointYMax, ptYMax);
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        if (v1.Pos().FuzzyEq(v2.Pos())) return new(v1.X, v1.Y, v1.X, v1.Y);

        var (arcRadius, arcCenter) = SegArcRadiusAndCenter(v1, v2);
        var startAngle = BaseMath.Angle(arcCenter, v1.Pos());
        var endAngle = BaseMath.Angle(arcCenter, v2.Pos());
        var sweepAngle = BaseMath.DeltaAngleSigned(startAngle, endAngle, v1.BulgeIsNeg());

        bool CrossesAngle(T angle)
        {
            return BaseMath.AngleIsWithinSweep(angle, startAngle, sweepAngle);
        }

        var minX = CrossesAngle(T.Pi) ? arcCenter.X - arcRadius : T.Min(v1.X, v2.X);
        var minY = CrossesAngle(T.CreateChecked(1.5) * T.Pi) ? arcCenter.Y - arcRadius : T.Min(v1.Y, v2.Y);
        var maxX = CrossesAngle(T.Zero) ? arcCenter.X + arcRadius : T.Max(v1.X, v2.X);
        var maxY = CrossesAngle(T.CreateChecked(0.5) * T.Pi) ? arcCenter.Y + arcRadius : T.Max(v1.Y, v2.Y);
    ...
    public static T SegLength<T>(PlineVertex<T> v1, PlineVertex<T> v2)
        where T : struct, IFloatingPointIeee754<T>
    {
        if (v1.FuzzyEq(v2)) return T.Zero;

        if (v1.BulgeIsZero()) return T.Sqrt(BaseMath.DistSquared(v1.Pos(), v2.Pos()));
    ...
    public static Vector2<T> SegMidpoint<T>(PlineVertex<T> v1, PlineVertex<T> v2)
        where T : struct, IFloatingPointIeee754<T>
    {
        if (v1.BulgeIsZero()) return BaseMath.Midpoint(v1.Pos(), v2.Pos());

        var (arcRadius, arcCenter) = SegArcRadiusAndCenter(v1, v2);
        var angle1 = BaseMath.Angle(arcCenter, v1.Pos());
        var angle2 = BaseMath.Angle(arcCenter, v2.Pos());
        var two = T.CreateChecked(2);
        var angleOffset = BaseMath.DeltaAngleSigned(angle1, angle2, v1.BulgeIsNeg()) / two;
        var midAngle = angle1 + angleOffset;
