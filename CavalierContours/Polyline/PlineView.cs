    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    public PlineVertex<T> Get(int index)
    {
        ...
        if (v == null) throw new ArgumentOutOfRangeException(nameof(index), $"Index {index} is out of view range");
        return v.Value;
    }
}
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    /// </param>
    public PlineViewData(int startIndex, int endIndexOffset, PlineVertex<T> updatedStart, T updatedEndBulge,
        Vector2<T> endPoint, bool invertedDirection)
    {
    ...
    public PlineVertex<T>? GetVertex(IPlineSource<T> source, int index)
    {
        ...
        {
            if (index == 0) return PlineVertex<T>.FromVector2(EndPoint, -UpdatedEndBulge);

            if (index < EndIndexOffset)
            {
                var bulgeI = source.FwdWrappingIndex(StartIndex, EndIndexOffset - index);
                var i = source.NextWrappingIndex(bulgeI);
                return source.Get(i).WithBulge(-source.Get(bulgeI).Bulge);
            }

            if (index == EndIndexOffset)
            {
                var i = source.FwdWrappingIndex(StartIndex, EndIndexOffset - index + 1);
                return source.Get(i).WithBulge(-UpdatedStart.Bulge);
            }

            if (index == EndIndexOffset + 1) return UpdatedStart.WithBulge(T.Zero);
        }
        else
        {
            if (index == 0) return UpdatedStart;

            if (index < EndIndexOffset)
            {
                var i = source.FwdWrappingIndex(StartIndex, index);
                return source.Get(i);
            }

            if (index == EndIndexOffset)
            {
                var i = source.FwdWrappingIndex(StartIndex, EndIndexOffset);
                return source.Get(i).WithBulge(UpdatedEndBulge);
            }

            if (index == EndIndexOffset + 1) return PlineVertex<T>.FromVector2(EndPoint, T.Zero);
        }
    ...
    public static PlineViewData<T>? CreateOnSingleSegment(
        ...
        T posEqualEps)
    {
        if (updatedStart.Pos().FuzzyEqEps(endIntersect, posEqualEps)) return null;
        ...
        T posEqualEps)
    {
        ...
        var currentVertex = source.Get(intersectIndex);
        int endIndexOffset;
        T updatedEndBulge;
        ...
        {
            var nextIndex = source.NextWrappingIndex(intersectIndex);
        ...
        return new(startIndex, endIndexOffset, updatedStart, updatedEndBulge, endIntersect, false);
    }
    ...
    public static PlineViewData<T> FromEntirePline(IPlineSource<T> source)
    {
        ...
        var vc = source.VertexCount;
        // Upstream uses assert!, active in release builds.
        if (vc < 2) throw new InvalidOperationException("source must have at least 2 vertexes to form view data");

        if (source.IsClosed) return new(0, vc - 1, source.Get(0), source.Get(vc - 1).Bulge, source.Get(0).Pos(), false);

        return new(0, vc - 2, source.Get(0), source.Get(vc - 2).Bulge, source.Get(vc - 1).Pos(), false);
    }
        ...
        T posEqualEps)
    {
        ...
        if (!source.IsClosed)
        {
            ...
            return FromSlicePoints(source, startPoint, startIndex, last.Value.Pos(), source.VertexCount - 1,
                posEqualEps);
        }
        ...
        var nextIdx = source.NextWrappingIndex(startIndex);
        if (source.Get(nextIdx).Pos().FuzzyEqEps(startPoint, posEqualEps)) startIndex = nextIdx;

        var startV1 = source.Get(startIndex);
        var startV2 = source.Get(source.NextWrappingIndex(startIndex));
        var split = PlineSeg.SegSplitAtPoint(startV1, startV2, startPoint, posEqualEps);

        var endIndexOffset = startV1.Pos().FuzzyEqEps(startPoint, posEqualEps) ? vc - 1 : vc;
        var updatedEndBulge = startV1.Pos().FuzzyEqEps(startPoint, posEqualEps)
        ...
        T posEqualEps)
    {
        ...
        int traverseCount;
        var indexDist = source.FwdWrappingDist(startIndex, endIndex);
        if (indexDist == 0 && source.IsClosed && !startPoint.FuzzyEqEps(endPoint, posEqualEps))
        {
            var segStart = source.Get(startIndex).Pos();
            var dist1 = BaseMath.DistSquared(segStart, startPoint);
            var dist2 = BaseMath.DistSquared(segStart, endPoint);
        ...
        if (traverseCount == 0) return CreateOnSingleSegment(source, startIndex, updatedStart, endPoint, posEqualEps);
    ...
    public ViewDataValidation ValidateForSource(IPlineSource<T> source)
    {
        ...
        var validationEps = T.CreateChecked(1e-5);
        var onSegEps = T.CreateChecked(1e-3);
        ...
        if (!PointIsOnSegment(StartIndex, UpdatedStart.Pos())) return ViewDataValidation.UpdatedStartNotOnSegment;

        var endIdx = source.FwdWrappingIndex(StartIndex, EndIndexOffset);
        if (!PointIsOnSegment(endIdx, EndPoint)) return ViewDataValidation.EndPointNotOnSegment;
