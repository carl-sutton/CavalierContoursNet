    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    public static PlineSegIntr<T> TangentIntersect(Vector2<T> point)
    {
        return new(PlineSegIntrKind.TangentIntersect, point, default);
    }
    ...
    public static PlineSegIntr<T> OneIntersect(Vector2<T> point)
    {
        return new(PlineSegIntrKind.OneIntersect, point, default);
    }
    ...
    public static PlineSegIntr<T> TwoIntersects(Vector2<T> point1, Vector2<T> point2)
    {
        return new(PlineSegIntrKind.TwoIntersects, point1, point2);
    }
    ...
    public static PlineSegIntr<T> OverlappingLines(Vector2<T> point1, Vector2<T> point2)
    {
        return new(PlineSegIntrKind.OverlappingLines, point1, point2);
    }
    ...
    public static PlineSegIntr<T> OverlappingArcs(Vector2<T> point1, Vector2<T> point2)
    {
        return new(PlineSegIntrKind.OverlappingArcs, point1, point2);
    }
}
...
public static class PlineSegIntersection
{
    ...
    public static PlineSegIntr<T> Intersect<T>(
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        var vIsLine = v1.BulgeIsZero();
        var uIsLine = u1.BulgeIsZero();
        ...
        if (vIsLine && uIsLine)
        {
            ...
            switch (intrResult.Kind)
            {
                ...
                case LineLineIntrKind.TrueIntersect:
                    return PlineSegIntr<T>.OneIntersect(BaseMath.PointFromParametric(v1.Pos(), v2.Pos(),
                        intrResult.Seg1T));
        ...
        {
            var (arcRadius, arcCenter) = PlineSeg.SegArcRadiusAndCenter(a1, a2);
            ...
            var lineLength = (p1 - p0).Length();

            Vector2<T>? PointInSweep(T t)
            {
                if (!(t * lineLength).FuzzyInRange(T.Zero, lineLength, posEqualEps)) return null;

                var p = BaseMath.PointFromParametric(p0, p1, t);
                var withinSweep =
                    BaseMath.PointWithinArcSweep(arcCenter, a1.Pos(), a2.Pos(), a1.BulgeIsNeg(), p, posEqualEps);
            ...
            switch (intrResult.Kind)
            {
                ...
                case LineCircleIntrKind.TangentIntersect:
                    ...
                    return tangentPt.HasValue
                        ? PlineSegIntr<T>.TangentIntersect(tangentPt.Value)
                        : PlineSegIntr<T>.NoIntersect;
                case LineCircleIntrKind.TwoIntersects:
                    ...
                    var liesOnArcP0 = PointLiesOnArc(p0);
                    var liesOnArcP1 = PointLiesOnArc(p1);
        ...
        if (vIsLine) return ProcessLineArcIntr(v1.Pos(), v2.Pos(), u1, u2);

        if (uIsLine) return ProcessLineArcIntr(u1.Pos(), u2.Pos(), v1, v2);
        ...
        switch (ccIntr.Kind)
        {
            ...
            case CircleCircleIntrKind.TangentIntersect:
                ...
                return BothArcsSweepPoint(ccIntr.Point1)
                    ? PlineSegIntr<T>.TangentIntersect(ccIntr.Point1)
                    : PlineSegIntr<T>.NoIntersect;
            case CircleCircleIntrKind.TwoIntersects:
                var point1 = ccIntr.Point1;
                var point2 = ccIntr.Point2;
                ...
                var pt1InSweep = BothArcsSweepPoint(point1);
                var pt2InSweep = BothArcsSweepPoint(point2);
            ...
            case CircleCircleIntrKind.Overlapping:
                var sameDirectionArcs = v1.BulgeIsNeg() == u1.BulgeIsNeg();
                var arc1Start = BaseMath.NormalizeRadians(BaseMath.Angle(arc1Center, v1.Pos()));
                var arc1Sweep = BaseMath.AngleFromBulge(v1.Bulge);
                ...
                var arc1End = arc1Start + arc1Sweep;
                var arc2End = arc2Start + arc2Sweep;
                var avgRadius = (arc1Radius + arc2Radius) / T.CreateChecked(2);

                var touchAtStartOfArc1 = (avgRadius * BaseMath.DeltaAngle(arc1Start, arc2End)).FuzzyEqZero(posEqualEps);
                var touchAtStartOfArc2 = (avgRadius * BaseMath.DeltaAngle(arc2Start, arc1End)).FuzzyEqZero(posEqualEps);

                if (touchAtStartOfArc1 && touchAtStartOfArc2) return PlineSegIntr<T>.TwoIntersects(u1.Pos(), u2.Pos());
                if (touchAtStartOfArc1) return PlineSegIntr<T>.OneIntersect(v1.Pos());
                if (touchAtStartOfArc2) return PlineSegIntr<T>.OneIntersect(sameDirectionArcs ? u1.Pos() : u2.Pos());

                var arc2StartsInArc1 = BaseMath.AngleIsWithinSweep(arc2Start, arc1Start, arc1Sweep);
                var arc2EndsInArc1 = BaseMath.AngleIsWithinSweep(arc2End, arc1Start, arc1Sweep);

                if (arc2StartsInArc1 && arc2EndsInArc1) return PlineSegIntr<T>.OverlappingArcs(u1.Pos(), u2.Pos());
                if (arc2StartsInArc1)
                ...
                var arc1StartsInArc2 = BaseMath.AngleIsWithinSweep(arc1Start, arc2Start, arc2Sweep);
                if (arc1StartsInArc2)
                    return sameDirectionArcs
