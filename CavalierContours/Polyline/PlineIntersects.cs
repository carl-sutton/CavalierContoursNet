...
public static class PlineIntersects
{
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (vc == 2)
        {
            if (polyline.IsClosed)
                if (polyline.Get(0).Bulge.FuzzyEq(-polyline.Get(1).Bulge))
                    return visitor.VisitOverlappingIntr(new(
                        0,
                        1,
                        polyline.Get(0).Pos(),
                        polyline.Get(1).Pos()
        ...
        bool VisitIndexes(int i, int j, int k)
        {
            ...
            {
                if (!visitor.VisitOverlappingIntr(new(
                        i,
                        j,
                        v1.Pos(),
                        v2.Pos()
                ...
                switch (intr.Kind)
                {
                    ...
                    case PlineSegIntrKind.OneIntersect:
                        if (!intr.Point1.FuzzyEqEps(v2.Pos(), posEqualEps))
                            if (!visitor.VisitBasicIntr(new(i, j, intr.Point1)))
                                return false;
                    ...
                    case PlineSegIntrKind.TwoIntersects:
                        ...
                        if (!intr.Point2.FuzzyEqEps(v2.Pos(), posEqualEps))
                            if (!visitor.VisitBasicIntr(new(i, j, intr.Point2)))
                                return false;

                        break;
                    case PlineSegIntrKind.OverlappingLines:
                    case PlineSegIntrKind.OverlappingArcs:
                        if (!visitor.VisitOverlappingIntr(new(i, j, intr.Point1, intr.Point2))) return false;
                        break;
                }
            }
        ...
        if (polyline.IsClosed)
        {
            if (!VisitIndexes(vc - 2, vc - 1, 0)) return false;
            if (!VisitIndexes(vc - 1, 0, 1)) return false;
        }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        for (var index = 0; index < itemIndices.Length; index++)
        {
            var i = itemIndices[index];
            var aabb = itemBoxes[index];

            var j = polyline.NextWrappingIndex(i);
            var v1 = polyline.Get(i);
            var v2 = polyline.Get(j);
    ...
    ///     <see
    ///         cref="PlineBoolean.VisitIntersects{T}(IPlineSource{T}, IPlineSource{T}, ITwoPlinesIntersectVisitor{T}, FindIntersectsOptions{T})" />
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (pline1.VertexCount < 2 || pline2.VertexCount < 2) return PlineIntersectsCollection<T>.NewEmpty();
        ...
        if (visitor.PossibleDuplicates1.Count == 0 && visitor.PossibleDuplicates2.Count == 0) return result;
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        for (var idx = 1; idx < intersects.Count; idx++)
        {
        ...
        if (result.Count > 1)
        {
            ...
            if (lastSliceEnd.FuzzyEqEps(firstSliceBegin, posEqualEps))
            {
                ...
                var updatedEndIndexOffset = firstSlice.ViewData.EndIndexOffset + lastSlice.ViewData.EndIndexOffset;
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        {
            var hitJ = _polyline.NextWrappingIndex(hitI);
            if (_i == hitI || _i == hitJ || _j == hitI || _j == hitJ) return true;

            if (_visitedPairs.Contains((hitI, _i))) return true;
            ...
            switch (intrResult.Kind)
            {
                ...
                case PlineSegIntrKind.OneIntersect:
                    if (!SkipIntrAtEnd(intrResult.Point1))
                        if (!_visitor.VisitBasicIntr(new(_i, hitI, intrResult.Point1)))
                        {
                            Cf = false;
                            return false;
                        }
                ...
                case PlineSegIntrKind.TwoIntersects:
                    ...
                    if (!SkipIntrAtEnd(intrResult.Point2))
                        if (!_visitor.VisitBasicIntr(new(_i, hitI, intrResult.Point2)))
                        {
                            Cf = false;
                            return false;
                        }
                ...
                case PlineSegIntrKind.OverlappingArcs:
                    if (!SkipIntrAtEnd(intrResult.Point1))
                        if (!_visitor.VisitOverlappingIntr(new(_i, hitI, intrResult.Point1, intrResult.Point2)))
                        {
                            Cf = false;
                            return false;
                        }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        public bool VisitOverlappingIntr(PlineOverlappingIntersect<T> intr)
        {
            ...
            {
                Intrs.Add(new(intr.StartIndex1, intr.StartIndex2, intr.Point1));
                Intrs.Add(new(intr.StartIndex1, intr.StartIndex2, intr.Point2));
            }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        {
            var j1 = _pline1.NextWrappingIndex(i1);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
            ...
            in PlineIntersectVisitContext<T> pline2Context)
        {
            ...
            switch (intersect.Kind)
            {
                ...
                case PlineSegIntrKind.TangentIntersect:
                case PlineSegIntrKind.OneIntersect:
                    if (!SkipIntrAtEnd(intersect.Point1)) Result.BasicIntersects.Add(new(i1, i2, intersect.Point1));
                    break;
                case PlineSegIntrKind.TwoIntersects:
                    if (!SkipIntrAtEnd(intersect.Point1)) Result.BasicIntersects.Add(new(i1, i2, intersect.Point1));
                    if (!SkipIntrAtEnd(intersect.Point2)) Result.BasicIntersects.Add(new(i1, i2, intersect.Point2));
                    break;
                case PlineSegIntrKind.OverlappingLines:
                case PlineSegIntrKind.OverlappingArcs:
                    Result.OverlappingIntersects.Add(new(i1, i2, intersect.Point1, intersect.Point2));
