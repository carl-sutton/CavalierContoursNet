...
internal class OrAndStitchSelector : IStitchSelector
{
    ...
    {
        var isPline1Idx = currentSliceIdx < _starts.Pline2
                          || (currentSliceIdx >= _starts.Pline1Overlapping &&
                              currentSliceIdx < _starts.Pline2Overlapping);

        if (isPline1Idx)
        {
            foreach (var i in availableIdx)
    ...
    public static OrAndStitchSelector FromPrunedSlices<T>(PrunedSlices<T> prunedSlices)
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        return new(prunedSlices.Starts);
    }
}
...
public static class PlineBoolean
{
    ...
    ///     <see
    ///         cref="PlineIntersects.VisitIntersects{T}(IPlineSource{T}, IPlineSource{T}, ITwoPlinesIntersectVisitor{T}, FindIntersectsOptions{T})" />
    ///     ,
    ...
    ///     <paramref name="pline1" /> or <paramref name="pline2" /> or
    ///     <paramref name="visitor" /> or <paramref name="options" /> is null.
    /// </exception>
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (pline1.VertexCount < 2 || pline2.VertexCount < 2) return;
        ...
        foreach (var (i2, j2) in pline2.IterSegmentIndexes())
        {
            ...
            pline1AabbIndex.VisitQueryWithStack(
                ...
                i1 =>
                {
                    var j1 = pline1.NextWrappingIndex(i1);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (pline1.VertexCount < 2 || pline2.VertexCount < 2) return result;

        var posEqualEps = options.PosEqualEps;
        var possibleDuplicates1 = new HashSet<int>();
        var possibleDuplicates2 = new HashSet<int>();

        var visitor = new FindIntersectsVisitor<T>(pline1, pline2, posEqualEps, result, possibleDuplicates1,
            possibleDuplicates2);
        VisitIntersects(pline1, pline2, visitor, options);

        if (possibleDuplicates1.Count == 0 && possibleDuplicates2.Count == 0) return result;
        ...
        foreach (var intr in result.BasicIntersects)
        {
            if (possibleDuplicates1.Contains(intr.StartIndex1))
            {
                ...
                if (intr.Point.FuzzyEqEps(startPt1, posEqualEps)) continue;
            }

            if (possibleDuplicates2.Contains(intr.StartIndex2))
            {
                var startPt2 = pline2.Get(intr.StartIndex2).Pos();
                if (intr.Point.FuzzyEqEps(startPt2, posEqualEps)) continue;
            }
    ...
    /// </remarks>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="intersects" /> or <paramref name="pline1" /> or
    ///     <paramref name="pline2" /> is null.
    /// </exception>
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (intersects.Count == 0) return result;

        intersects.Sort((intrA, intrB) =>
        {
            var cmp = intrA.StartIndex2.CompareTo(intrB.StartIndex2);
            if (cmp != 0) return cmp;

            var start = pline2.Get(intrA.StartIndex2).Pos();
            var dist1 = BaseMath.DistSquared(start, intrA.Point1);
            var dist2 = BaseMath.DistSquared(start, intrB.Point1);
    ...
    private static void AdjustSpEpIndexes<T>(
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        var spIdxNext = pline.NextWrappingIndex(spIdx);
        if (sp.FuzzyEqEps(pline.Get(spIdxNext).Pos(), posEqualEps)) spIdx = spIdxNext;
        var epIdxNext = pline.NextWrappingIndex(epIdx);
        if (ep.FuzzyEqEps(pline.Get(epIdxNext).Pos(), posEqualEps)) epIdx = epIdxNext;
    }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        if (useSecondIndex)
        {
            ...
            foreach (var overlappingSlice in booleanInfo.OverlappingSlices)
            {
                ...
                var spIdx = overlappingSlice.StartIndexes.Second;
                var epIdx = overlappingSlice.EndIndexes.Second;
        ...
        else
        {
            ...
            foreach (var overlappingSlice in booleanInfo.OverlappingSlices)
            {
                var sp = overlappingSlice.ViewData.UpdatedStart.Pos();
                var ep = overlappingSlice.ViewData.EndPoint;
                var spIdx = overlappingSlice.StartIndexes.First;
                var epIdx = overlappingSlice.EndIndexes.First;
                AdjustSpEpIndexes(pline, ref spIdx, sp, ref epIdx, ep, posEqualEps);

                var spIsSliceStart = !overlappingSlice.OpposingDirections;
                ...
                listSp.Add(new(sp, spIsSliceStart));
                ...
                listEp.Add(new(ep, !spIsSliceStart));
            }
        }
        ...
        foreach (var pair in intersectsLookup)
        {
            ...
            {
                var dist1 = BaseMath.DistSquared(intr1.Pos, startPos);
                var dist2 = BaseMath.DistSquared(intr2.Pos, startPos);
                return dist1.CompareTo(dist2);
            });
        }
        ...
        foreach (var pair in intersectsLookup)
        {
            ...
            var nextIndex = pline.NextWrappingIndex(startIndex);
            ...
            if (intrsList.Count != 1)
            {
                ...
                for (var i = 1; i < intrsList.Count; i++)
                {
                    var split = PlineSeg.SegSplitAtPoint(prevVertex, endVertex, intrsList[i].Pos, posEqualEps);
                    prevVertex = split.SplitVertex;

                    if (intrsList[i - 1].IsStartOfOverlappingSlice) continue;

                    if (split.UpdatedStart.Pos().FuzzyEqEps(split.SplitVertex.Pos(), posEqualEps)) continue;

                    var midpoint = PlineSeg.SegMidpoint(split.UpdatedStart, split.SplitVertex);
                    if (!pointOnSlicePred(midpoint)) continue;
            ...
            if (lastIntr.IsStartOfOverlappingSlice) continue;

            var sliceStartVertex =
                PlineSeg.SegSplitAtPoint(startVertex, endVertex, lastIntr.Pos, posEqualEps).SplitVertex;

            var index = nextIndex;
            var loopCount = 0;
            var maxLoopCount = pline.VertexCount;
            while (true)
            {
                if (loopCount > maxLoopCount)
                    throw new InvalidOperationException(
                        "loopCount exceeded maxLoopCount while creating slices from intersects");
                loopCount++;
                ...
                if (intersectsLookup.TryGetValue(index, out var nextIntrList))
                {
                    ...
                    if (pointOnSlicePred(midpoint)) outputSlices.Add(slice);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var startOfPline2Slices = slicesRemaining.Count;
        ...
        var startOfPline1OverlappingSlices = slicesRemaining.Count;
        ...
        var startOfPline2OverlappingSlices = slicesRemaining.Count;

        foreach (var s in booleanInfo.OverlappingSlices)
            slicesRemaining.Add(BooleanPlineSlice<T>.FromOverlapping(pline2, s, false));

        var setOpposingDirection = mode switch
        {
        ...
        if (setOpposingDirection != booleanInfo.OpposingDirections())
            for (var i = 0; i < startOfPline2Slices; i++)
            {
                ...
                slicesRemaining[i] = new(updatedViewData, s.SourceIsPline1, s.Overlapping);
            }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        Action<O, List<BooleanPlineSlice<T>>> closePline = (pline, subslices) =>
        {
            ...
            if (pline.VertexCount < 3) return;

            pline.Remove(pline.VertexCount - 1);
            pline.SetIsClosed(true);

            if (collapsedAreaEps.HasValue && T.Abs(pline.Area()) < collapsedAreaEps.Value) return;

            result.Add(new(pline, subslices));
        };
        ...
        for (var i = 0; i < slices.Count; i++)
        {
            ...
            var beginningSliceIdx = i;
            var currentSliceIdx = i;
            var loopCount = 0;
            ...
            while (true)
            {
                if (loopCount > maxLoopCount)
                    throw new InvalidOperationException(
                        "loopCount exceeded maxLoopCount while creating closed polylines from slices");
                loopCount++;
                ...
                aabbIndex.VisitQueryWithStack(
                    ...
                    idx =>
                    {
                        if (idx == beginningSliceIdx || !visitedSliceIdx[idx]) queryResults.Add(idx);
                        return true;
                    },
                    queryStack
                );

                if (queryResults.Count == 0) break;
                ...
                if (selected.Value == beginningSliceIdx)
                {
                    closePline(currentPline, subslices);
                    break;
                }

                var connectedSliceIdx = selected.Value;
        ...
        foreach (var resultItem in result) resultItem.Pline.SetUserDataValues(compositeUserdata);
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        var isPline1InPline2 = () => pline2.WindingNumber(pline1.Get(0).Pos()) != 0;
        var isPline2InPline1 = () => pline1.WindingNumber(pline2.Get(0).Pos()) != 0;

        var posEqualEps = options.PosEqualEps;
        var collapsedAreaEps = options.CollapsedAreaEps;
        ...
        switch (operation)
        {
            case BooleanOp.Or:
            {
                ...
                if (!booleanInfo.AnyIntersects())
                {
                    ...
                    return BooleanResult<O, T>.FromWholePlines(
                        new[]
                        {
                            PlineSourceExtensions.CreateFrom<O, T>(pline1),
                            PlineSourceExtensions.CreateFrom<O, T>(pline2)
                        },
                        Array.Empty<O>(),
                ...
                return new(posPlines, negPlines, BooleanResultInfo.Intersected);
            }
            ...
            case BooleanOp.And:
            {
                ...
                return new(posPlines, new(), BooleanResultInfo.Intersected);
            }
            ...
            case BooleanOp.Not:
            {
                ...
                if (!booleanInfo.AnyIntersects())
                {
                    if (isPline1InPline2()) return BooleanResult<O, T>.Empty(BooleanResultInfo.Pline1InsidePline2);
            ...
            case BooleanOp.Xor:
            {
                ...
                return new(remaining1, new(), BooleanResultInfo.Intersected);
            }
        ...
        where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        ...
        public bool Visit(PlineSegIntr<T> intersect, in PlineIntersectVisitContext<T> pline1Context,
            in PlineIntersectVisitContext<T> pline2Context)
        {
            var i1 = pline1Context.VertexIndex;
            var i2 = pline2Context.VertexIndex;
            ...
            switch (intersect.Kind)
            {
                ...
                case PlineSegIntrKind.OneIntersect:
                {
                    ...
                    if (!SkipIntrAtEnd(point)) Result.BasicIntersects.Add(new(i1, i2, point));
                    break;
                }
                case PlineSegIntrKind.TwoIntersects:
                {
                    ...
                    if (!SkipIntrAtEnd(point1)) Result.BasicIntersects.Add(new(i1, i2, point1));
                    if (!SkipIntrAtEnd(point2)) Result.BasicIntersects.Add(new(i1, i2, point2));
                    break;
                }
                ...
                case PlineSegIntrKind.OverlappingArcs:
                {
                    var point1 = intersect.Point1;
                    var point2 = intersect.Point2;
                    Result.OverlappingIntersects.Add(new(i1, i2, point1, point2));
