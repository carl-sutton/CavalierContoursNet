    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    {
        if (slice.Length != 3) throw new ArgumentException("Slice must contain exactly 3 elements", nameof(slice));
        return new(slice[0], slice[1], slice[2]);
    }
    ...
    {
        return new(vector.X, vector.Y, bulge);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool BulgeIsZero()
    {
        return Bulge.FuzzyEqZero();
    }
    ...
    public bool Equals(PlineVertex<T> other)
    {
        return X == other.X && Y == other.Y && Bulge == other.Bulge;
    }
    ...
    {
        return obj is PlineVertex<T> other && Equals(other);
    }
    ...
    {
        return HashCode.Combine(X, Y, Bulge);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool operator ==(PlineVertex<T> left, PlineVertex<T> right)
    {
