    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    public BooleanResult(List<BooleanResultPline<P, T>> posPlines, List<BooleanResultPline<P, T>> negPlines,
        BooleanResultInfo resultInfo)
    {
        _posPlines = new(posPlines);
        _negPlines = new(negPlines);
    ...
    public static BooleanResult<P, T> FromWholePlines(IEnumerable<P> posPlines, IEnumerable<P> negPlines,
        BooleanResultInfo resultInfo)
    {
        ...
        foreach (var p in posPlines) pos.Add(new(p, new()));

        var neg = new List<BooleanResultPline<P, T>>();
        foreach (var p in negPlines) neg.Add(new(p, new()));

        return new(pos, neg, resultInfo);
    }
}
    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    {
        return new(new PlineBasicIntersect<T>(startIndex1, startIndex2, point));
    }
    ...
    /// </returns>
    public static PlineIntersect<T> NewOverlapping(int startIndex1, int startIndex2, Vector2<T> point1,
        Vector2<T> point2)
    {
        return new(new PlineOverlappingIntersect<T>(startIndex1, startIndex2, point1, point2));
    }
}
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    /// <returns><c>true</c> to continue visiting, <c>false</c> to stop the traversal early.</returns>
    bool Visit(PlineSegIntr<T> intersect, in PlineIntersectVisitContext<T> pline1Context,
    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    ///     <paramref name="basicIntersects" /> or
    ///     <paramref name="overlappingIntersects" /> is null.
    /// </exception>
    public PlineIntersectsCollection(List<PlineBasicIntersect<T>> basicIntersects,
        List<PlineOverlappingIntersect<T>> overlappingIntersects)
    {
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
        ...
        T posEqualEps)
    {
        ...
        if (endIntr == null)
        {
            ...
            return new(startIndexes, startIndexes, viewData, false, opposingDirections);
        }
        ...
        if (endIntrVal.Point2.FuzzyEqEps(startIntr.Point1, posEqualEps))
        {
            ...
            return new(startIndexes, startIndexes, viewData, true, opposingDirections);
        }

        {
            ...
            if (startIntr.StartIndex2 == endIntrVal.StartIndex2)
            {
                ...
                return new(startIndexes, endIndexes, viewData, false, opposingDirections);
            }
            else
            {
                var updatedStart = PlineSeg.SegSplitAtPoint(startU1, startU2, startIntr.Point1, posEqualEps)
                    .SplitVertex;
    ...
    where T : struct, IFloatingPointIeee754<T>, IMinMaxValue<T>
{
    ...
    public static BooleanPlineSlice<T> FromOpenPlineSlice(in PlineViewData<T> data, bool sourceIsPline1, bool inverted)
    {
        ...
        return new(viewData, sourceIsPline1, false);
    }
    ...
    /// </returns>
    public static BooleanPlineSlice<T> FromOverlapping(IPlineSource<T> source, in OverlappingSlice<T> overlappingSlice,
        bool inverted)
    {
        var viewData = new PlineViewData<T>(
        ...
        return new(viewData, false, true);
    }
