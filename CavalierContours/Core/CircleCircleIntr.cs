    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    public static CircleCircleIntr<T> TangentIntersect(Vector2<T> point)
    {
        return new(CircleCircleIntrKind.TangentIntersect, point, default);
    }
    ...
    public static CircleCircleIntr<T> TwoIntersects(Vector2<T> point1, Vector2<T> point2)
    {
        return new(CircleCircleIntrKind.TwoIntersects, point1, point2);
    }
    ...
    {
        return obj is CircleCircleIntr<T> other && Equals(other);
    }
    ...
    {
        return HashCode.Combine(Kind, Point1, Point2);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool operator ==(CircleCircleIntr<T> left, CircleCircleIntr<T> right)
    {
...
public static class CircleCircleIntersection
{
    ...
    public static CircleCircleIntr<T> Intersect<T>(
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        var cv = center2 - center1;
        var d2 = cv.Dot(cv);
        var d = T.Sqrt(d2);

        if (d.FuzzyEqZero(epsilon))
        {
            if (radius1.FuzzyEq(radius2, epsilon)) return CircleCircleIntr<T>.Overlapping;
        ...
        var rad1Sq = radius1 * radius1;
        var two = T.CreateChecked(2);
        var a = (rad1Sq - radius2 * radius2 + d2) / (two * d);
        var midpoint = center1 + cv.Scale(a / d);
        var diff = rad1Sq - a * a;

        if (diff < T.Zero) return CircleCircleIntr<T>.TangentIntersect(midpoint);

        var h = T.Sqrt(diff);
        var hOverD = h / d;
        var xTerm = hOverD * cv.Y;
        var yTerm = hOverD * cv.X;

        Vector2<T> pt1 = new(midpoint.X + xTerm, midpoint.Y - yTerm);
        Vector2<T> pt2 = new(midpoint.X - xTerm, midpoint.Y + yTerm);

        if (pt1.FuzzyEqEps(pt2, epsilon)) return CircleCircleIntr<T>.TangentIntersect(pt1);
