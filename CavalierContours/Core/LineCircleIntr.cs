    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    public static LineCircleIntr<T> TangentIntersect(T t0)
    {
        return new(LineCircleIntrKind.TangentIntersect, t0, default);
    }
    ...
    public static LineCircleIntr<T> TwoIntersects(T t0, T t1)
    {
        return new(LineCircleIntrKind.TwoIntersects, t0, t1);
    }
    ...
    {
        return obj is LineCircleIntr<T> other && Equals(other);
    }
    ...
    {
        return HashCode.Combine(Kind, T0, T1);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool operator ==(LineCircleIntr<T> left, LineCircleIntr<T> right)
    {
...
public static class LineCircleIntersection
{
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        var dx = p1.X - p0.X;
        var dy = p1.Y - p0.Y;
        var h = circleCenter.X;
        var k = circleCenter.Y;
        ...
        if (p0.FuzzyEqEps(p1, epsilon))
        {
            ...
            var xh = (p0.X + p1.X) / two - h;
            var yk = (p0.Y + p1.Y) / two - k;
            if (T.Sqrt(xh * xh + yk * yk).FuzzyEq(radius, epsilon)) return LineCircleIntr<T>.TangentIntersect(T.Zero);
            return LineCircleIntr<T>.NoIntersect;
        }

        var p0Shifted = p0 - circleCenter;
        var p1Shifted = p1 - circleCenter;

        var (a, b, c) = dx.FuzzyEqZero()
        ...
        var shortestDist = T.Abs(c) / T.Sqrt(a2_b2);

        if (shortestDist > radius + epsilon) return LineCircleIntr<T>.NoIntersect;
        ...
        {
            var tangentT = BaseMath.ParametricFromPoint(p0, p1, new(x0, y0), epsilon);
        ...
        var sol1 = BaseMath.ParametricFromPoint(p0, p1, point1, epsilon);
        var sol2 = BaseMath.ParametricFromPoint(p0, p1, point2, epsilon);

        var (t0, t1) = BaseMath.MinMax(sol1, sol2);
