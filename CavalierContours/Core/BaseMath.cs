...
public static class BaseMath
{
    ...
    {
        var tau = T.Tau;
        if (angle >= T.Zero && angle <= tau) return angle;
    ...
    {
        var diff = NormalizeRadians(angle2 - angle1);
        if (diff > T.Pi) diff -= T.Tau;
        return diff;
    }
    ...
    {
        var diff = DeltaAngle(angle1, angle2);
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool AngleIsBetweenEps<T>(T testAngle, T startAngle, T endAngle, T epsilon)
        where T : struct, IFloatingPointIeee754<T>
    {
        var endSweep = NormalizeRadians(endAngle - startAngle);
        var midSweep = NormalizeRadians(testAngle - startAngle);
    ...
    /// <returns><see langword="true" /> if the test angle lies within the sweep.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool AngleIsBetween<T>(T testAngle, T startAngle, T endAngle)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// <returns><see langword="true" /> if the test angle lies within the sweep.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool AngleIsWithinSweepEps<T>(T testAngle, T startAngle, T sweepAngle, T epsilon)
        where T : struct, IFloatingPointIeee754<T>
    {
        var endAngle = startAngle + sweepAngle;
        if (sweepAngle < T.Zero) return AngleIsBetweenEps(testAngle, endAngle, startAngle, epsilon);
    ...
    /// <returns><see langword="true" /> if the test angle lies within the sweep.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool AngleIsWithinSweep<T>(T testAngle, T startAngle, T sweepAngle)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static (T, T) QuadraticSolutions<T>(T a, T b, T c, T sqrtDiscriminant)
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        var denom = two * a;
        var sol1 = b < T.Zero ? (-b + sqrtDiscriminant) / denom : (-b - sqrtDiscriminant) / denom;
        var sol2 = c / a / sol1;
        return (sol1, sol2);
    }
    ...
    {
        var two = T.CreateChecked(2);
        return new((p0.X + p1.X) / two, (p0.Y + p1.Y) / two);
    }
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector2<T> PointOnCircle<T>(T radius, Vector2<T> center, T angle)
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        var s = T.Sin(angle);
        var c = T.Cos(angle);
        return new(center.X + radius * c, center.Y + radius * s);
    }
    ...
    /// <param name="p1">End point of the segment.</param>
    /// <param name="t">
    ///     Parametric value, <c>0</c> at <paramref name="p0" /> and <c>1</c> at
    ///     <paramref name="p1" />.
    /// </param>
    /// <returns><c>p0 + t * (p1 - p0)</c>.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector2<T> PointFromParametric<T>(Vector2<T> p0, Vector2<T> p1, T t)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static T ParametricFromPoint<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point, T epsilon)
        where T : struct, IFloatingPointIeee754<T>
    {
        var xDiff = p1.X - p0.X;
        var yDiff = p1.Y - p0.Y;
        ...
        if (T.Abs(xDiff) < T.Abs(yDiff)) return (point.Y - p0.Y) / yDiff;
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector2<T> LineSegClosestPoint<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point)
        where T : struct, IFloatingPointIeee754<T>
    {
        var v = p1 - p0;
        var w = point - p0;
        var c1 = w.Dot(v);
        if (c1 < Fuzzy<T>.Epsilon) return p0;

        var c2 = v.LengthSquared();
        if (c2 < c1 + Fuzzy<T>.Epsilon) return p1;
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static T PerpDotTestValue<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsLeft<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsLeftOrEqual<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point)
        where T : struct, IFloatingPointIeee754<T>
    {
        return PerpDotTestValue(p0, p1, point) >= T.Zero;
    }
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsLeftOrCoincidentEps<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point, T epsilon)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsLeftOrCoincident<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsRightOrCoincidentEps<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point, T epsilon)
        where T : struct, IFloatingPointIeee754<T>
    {
        Debug.Assert(epsilon > T.Zero);
        return PerpDotTestValue(p0, p1, point) < epsilon;
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsRightOrCoincident<T>(Vector2<T> p0, Vector2<T> p1, Vector2<T> point)
        where T : struct, IFloatingPointIeee754<T>
    {
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool PointWithinArcSweep<T>(Vector2<T> center, Vector2<T> arcStart, Vector2<T> arcEnd,
        bool isClockwise, Vector2<T> point, T epsilon) where T : struct, IFloatingPointIeee754<T>
    {
        Debug.Assert(epsilon > T.Zero);
        ...
        if (pointVector.LengthSquared() < epsilon * epsilon) return true;

        // Construct the sweep boundary vectors and determine which side of each one the
        // point lies on.
        var startVector = arcStart - center;
        var endVector = arcEnd - center;
        var startCross = startVector.PerpDot(pointVector);
        var endCross = endVector.PerpDot(pointVector);

        // First test the whole sweep region without tolerance.
        var exactlyWithinSweep = isClockwise
            ? startCross <= T.Zero && endCross >= T.Zero
            : startCross >= T.Zero && endCross <= T.Zero;
        if (exactlyWithinSweep) return true;
        ...
        {
            return ray.Dot(pointVector) >= T.Zero && T.Abs(cross) < epsilon * ray.Length();
        }
    }
