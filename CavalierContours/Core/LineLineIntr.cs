    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    public static LineLineIntr<T> TrueIntersect(T seg1T, T seg2T)
    {
        return new(LineLineIntrKind.TrueIntersect, seg1T, seg2T, default);
    }
    ...
    public static LineLineIntr<T> FalseIntersect(T seg1T, T seg2T)
    {
        return new(LineLineIntrKind.FalseIntersect, seg1T, seg2T, default);
    }
    ...
    public static LineLineIntr<T> Overlapping(T seg2T0, T seg2T1)
    {
        return new(LineLineIntrKind.Overlapping, default, seg2T0, seg2T1);
    }
    ...
    public T OverlapSeg2T0
    {
        ...
        get
        {
            Debug.Assert(Kind == LineLineIntrKind.Overlapping,
                "OverlapSeg2T0 is only meaningful for Overlapping results.");
            return Seg2T;
        }
    }
    ...
    public T OverlapSeg2T1
    {
        ...
        {
            Debug.Assert(Kind == LineLineIntrKind.Overlapping,
                "OverlapSeg2T1 is only meaningful for Overlapping results.");
            return Seg2T1;
        }
    }
    ...
    {
        return obj is LineLineIntr<T> other && Equals(other);
    }
    ...
    {
        return HashCode.Combine(Kind, Seg1T, Seg2T, Seg2T1);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool operator ==(LineLineIntr<T> left, LineLineIntr<T> right)
    {
...
public static class LineLineIntersection
{
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        var v = v2 - v1;
        var u = u2 - u1;
        var vPdotU = v.PerpDot(u);
        var w = v1 - u1;

        var seg1Length = (v2 - v1).Length();
        var seg2Length = (u2 - u1).Length();

        if (!vPdotU.FuzzyEqZero(epsilon))
        {
            var seg1T = u.PerpDot(w) / vPdotU;
            var seg2T = v.PerpDot(w) / vPdotU;
        ...
        var vPdotW = v.PerpDot(w);
        var uPdotW = u.PerpDot(w);

        if (!vPdotW.FuzzyEqZero(epsilon) || !uPdotW.FuzzyEqZero(epsilon)) return LineLineIntr<T>.NoIntersect;

        var vIsPoint = v1.FuzzyEqEps(v2, epsilon);
        var uIsPoint = u1.FuzzyEqEps(u2, epsilon);

        if (vIsPoint && uIsPoint)
        {
            if (v1.FuzzyEqEps(u1, epsilon)) return LineLineIntr<T>.TrueIntersect(T.Zero, T.Zero);
            return LineLineIntr<T>.NoIntersect;
        }

        if (vIsPoint)
        {
            var seg2T = BaseMath.ParametricFromPoint(u1, u2, v1, epsilon);
        ...
        if (uIsPoint)
        {
            var seg1T = BaseMath.ParametricFromPoint(v1, v2, u1, epsilon);
            if ((seg1T * seg1Length).FuzzyInRange(T.Zero, seg1Length, epsilon))
        ...
        {
            var seg1T = v1.FuzzyEqEps(u1, epsilon) || v1.FuzzyEqEps(u2, epsilon) ? T.Zero : T.One;
